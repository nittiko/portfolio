...
function validateAll() {
  ...
  if (result) {
    ocument.getElementById("signUp").submit();
  }
...
function sendConfirmToken() {
  ...
  fetch("/signUp/validateDuplicateEmail", {
    method: "POST",
    body: formData,
  }).then((response) => {
    if (response.status === 409) {
      ...
      localStorage.setItem("confirmToken", response.body.text);
      console.log(localStorage.setItem("confirmToken", response.body.text));
      // 인증 번호 입력 폼 표시
...
function confirmEmailToken() {
  // 입력한 인증 번호
  const confirmToken1 = document.getElementById("confirmToken").textContent;
  // 저장된 인증 번호
  const confirmToken2 = localStorage.getItem("confirmToken");
